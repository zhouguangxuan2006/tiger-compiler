...
assem::Proc *ProcEntryExit3(frame::Frame *frame, assem::InstrList *body) {
  const std::array<X64RegManager::Reg, 5> saved_regs{
      X64RegManager::RBX, X64RegManager::R12, X64RegManager::R13,
      X64RegManager::R14, X64RegManager::R15};
  int save_area_size = saved_regs.size() * frame->word_size_;
  int save_area_offset = frame->outgoing_size_;
  int frame_size = -frame->local_offset_ + frame->outgoing_size_ + save_area_size;
  ...
  for (std::size_t i = 0; i < saved_regs.size(); ++i) {
    prolog << "movq " << *reg_manager->temp_map_->Look(
                             reg_manager->GetRegister(saved_regs[i]))
           << ", " << save_area_offset + i * frame->word_size_ << "(%rsp)\n";
  }

  std::ostringstream epilog;
  for (std::size_t i = 0; i < saved_regs.size(); ++i) {
    epilog << "movq " << save_area_offset + i * frame->word_size_ << "(%rsp), "
           << *reg_manager->temp_map_->Look(
                  reg_manager->GetRegister(saved_regs[i]))
           << "\n";
  }
