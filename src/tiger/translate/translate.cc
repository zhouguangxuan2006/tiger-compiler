...
tr::ExpAndTy *CallExp::Translate(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  // Built-in runtime functions have no Tiger label.
...
tr::ExpAndTy *OpExp::Translate(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  bool string_eq =
      (oper_ == EQ_OP || oper_ == NEQ_OP) &&
      (typeid(*left_exp_ty->ty_->ActualTy()) == typeid(type::StringTy) ||
       typeid(*right_exp_ty->ty_->ActualTy()) == typeid(type::StringTy));
  if (string_eq) {
    auto *args = new tree::ExpList();
    args->Append(left_exp);
    args->Append(right_exp);
    tree::Exp *equal = frame::ExternalCall("string_equal", args);
    tree::RelOp relop = oper_ == EQ_OP ? tree::NE_OP : tree::EQ_OP;
    auto *cjump = new tree::CjumpStm(relop, equal, new tree::ConstExp(0),
                                     nullptr, nullptr);
    tr::PatchList trues({&cjump->true_label_});
    tr::PatchList falses({&cjump->false_label_});
  ...
  if (oper_ == EQ_OP || oper_ == NEQ_OP || oper_ == LT_OP || 
    ...
    // 创建 Cx 表达式（条件跳转）
    auto *cjump = new tree::CjumpStm(relop, left_exp, right_exp, nullptr, nullptr);
    tr::PatchList trues({&cjump->true_label_});
    tr::PatchList falses({&cjump->false_label_});
...
tr::ExpAndTy *RecordExp::Translate(env::VEnvPtr venv, env::TEnvPtr tenv,
  ...
  tree::Stm *alloc_stm = new tree::MoveStm(
      ...
      new tree::CallExp(new tree::NameExp(temp::LabelFactory::NamedLabel("alloc_record")),
                        args));
